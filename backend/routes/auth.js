...
// import StudentId from "../models/StudentId.js";
...
//     // check if studentId exists in list
//     const sid = await StudentId.findOne({ studentId });
//     if (!sid) {
//       return res.status(400).json({ message: "Student ID not found. Contact your teacher." });
//     }
//     if (sid.registered) {
//       return res.status(400).json({ message: "This Student ID is already registered." });
//     }

//     // unique email/studentId in users
//     const existingUser = await User.findOne({ $or: [{ email }, { studentId }] });
//     if (existingUser) {
//       return res.status(400).json({ message: "Email or Student ID already registered as user." });
//     }

//     const salt = await bcrypt.genSalt(10);
//     const passwordHash = await bcrypt.hash(password, salt);

//     const newUser = new User({ name, email, studentId, passwordHash, role: "student" });
//     await newUser.save();

//     // mark student id as registered
//     sid.registered = true;
//     await sid.save();

//     const token = jwt.sign({ id: newUser._id, role: newUser.role, studentId: newUser.studentId }, process.env.JWT_SECRET, { expiresIn: "7d" });

//     return res.json({ token, user: { name: newUser.name, email: newUser.email, studentId: newUser.studentId }});
//   } catch (err) {
//     console.error(err);
...
//     if (!emailOrId || !password) return res.status(400).json({ message: "All fields required" });

//     // allow login with email OR studentId
//     const user = await User.findOne({ $or: [{ email: emailOrId }, { studentId: emailOrId }] });
//     if (!user) return res.status(400).json({ message: "User not found" });

//     const isMatch = await bcrypt.compare(password, user.passwordHash);
//     if (!isMatch) return res.status(400).json({ message: "Incorrect password" });

//     const token = jwt.sign({ id: user._id, role: user.role, studentId: user.studentId }, process.env.JWT_SECRET, { expiresIn: "7d" });

//     return res.json({ token, user: { name: user.name, email: user.email, studentId: user.studentId, role: user.role }});
//   } catch (err) {
//     console.error(err);
